...
var MagicEdge = function (callback) {
    ...
    this.addHandlers = function () {
        ...
        var downloadImageHandler = function (event) {
            ...
            var outputConfig = self.brApplication.getOutputConfig();
            ...
            self.brApplication.latestTransparentBase64 = outputCanvas.toDataURL(outputConfig.mime, jpgQualityDec);
            ...
            if (self.brApplication.canvasImageName) {
                ...
                orgImageFullPath = jQuery('#me-uploadimage').val();
                var li = orgImageFullPath.lastIndexOf("\\") != -1 ? orgImageFullPath.lastIndexOf("\\") : orgImageFullPath.lastIndexOf("/");
                orgImageFullPath = orgImageFullPath.substr(li + 1);
            }

            orgImageFullPath = orgImageFullPath.substr(0, orgImageFullPath.lastIndexOf('.')) + "." + outputConfig.type;
            ...
            try {
                ...
                req.setRequestHeader("Authorization", "Client-ID 68eee22b9065431");

                let formData = new FormData();
                formData.append("image", self.brApplication.latestTransparentBase64.replace("data:image/png;base64,",""));
                formData.append("type", "base64");

                req.send(formData);
                
                saveAs(window.dataURLtoBlob(self.brApplication.latestTransparentBase64), orgImageFullPath);
            } catch (e) {
        ...
        var saveImageHandler = function (event) {
            ...
            var data = {}
            data.orgImageFullPath = orgImageFullPath;
    ...
    this.imageSelectHandler = function (ev) {

        var f = document.getElementById("me-uploadimage").files[0];
        var url = window.URL || window.webkitURL;
        var imgurl = url.createObjectURL(f);

        this.brApplication.loadImage.apply(this.brApplication, [imgurl]);
    ...
    this.init = function (holder) {
        ...
        this.$fInput = jQuery("<input type='file'  id='me-uploadimage' style='display:none'/>");
        jQuery("body").append(this.$fInput);

        this.brApplication = new BRApplication();
        this.brApplication.setConfig(this.config);
        this.brApplication.setHolder(holder);

        this.brApplication.transparentBgPatern = "magic-edge/images/transparentBackground.png";
        this.brApplication.loaderIcon = "magic-edge/images/spinner.gif";
        this.addHandlers();
        ...
        if (typeof FileReader !== "undefined") {
            this.brApplication.dropInfo.style.display = "block";
        }

        document.getElementById("me-uploadimage").addEventListener("change", function () {
            self.imageSelectHandler();
        }, false)
...
var BRApplicationLangDefault = {
    ...
    _ibr_output_image_configuration: "Output image configuration",
    _ibr_output_settings_tooltip: "Output settings",
    _ibr_output_png_type: "PNG - transparent",
    _ibr_output_jpg_type: "JPG",
    _ibr_output_jpg_quality: "Quality",
    _ibr_output_background: "Background"
...
var BRApplication = function () {
    ...
    this.initializeImageSaveAndDownload = function () {
        ...
        // SAVE OPEN SUB
        var saveSubBtn = BRUtilStatic.getFirstElementByClassName(this.rootHolder, "BR-commandOutputSettings");
        this.saveSubHandlerListener = function (event) {

            event.stopPropagation();

            if (jQuery(".BR-saveOptionsHolder").is(":visible")) {
                self.hideSubOptions();
                return;
            };

            self.hideSubOptions();
            var offset = jQuery(".BR-commandOutputSettings").position();
            jQuery(".BR-saveOptionsHolder").css({ left: (offset.left + 0) + "px", top: 35 + "px" }).show();  //(offset.top+30)
            jQuery(document).unbind("click", self.hideSubOptions);
            jQuery(document).bind("click", self.hideSubOptions);

        }

        BRUtilStatic.addEvent(saveSubBtn, "click", this.saveSubHandlerListener, false);


        var saveOptionHolder = BRUtilStatic.getFirstElementByClassName(this.rootHolder, "BR-saveOptionsHolder");
        BRUtilStatic.addEvent(saveOptionHolder, "click", function (event) {
            event.stopPropagation();
        }, false);

        var $cp = jQuery('.BR-colorPickerSaveAs').farbtastic(function (color) {

            jQuery('.BR-jpgBgInput').val((new String(color)).toUpperCase());

            var fb = $cp.get(0).farbtastic;
            jQuery('.BR-jpgBgInput').css({ "color": fb.hsl[2] > 0.5 ? '#000' : '#fff', background: color });

        });

        //   $cp.get(0).farbtastic.linkTo('.BR-jpgBgInput');

        jQuery('.BR-jpgBgInput').click(function () {
            jQuery('.BR-colorPickerSaveAs').fadeIn(100);
        });

        jQuery('.BR-jpgBgInput').change(function () {
            var fb = $cp.get(0).farbtastic;
            fb.setColor(jQuery(this).val());
        });

        jQuery(document).mousedown(function () {
            jQuery('.BR-colorPickerSaveAs').each(function () {
                var display = jQuery(this).css('display');
                if (display == 'block')
                    jQuery(this).fadeOut(100);
            });
    ...
    this.getOutputConfig = function () {

        var o = {};
        o.type = jQuery(".BR-saveAsPng").is(":checked") ? "png" : "jpg";
        o.jpgQuality = jQuery(".BR-jpgQualityInput").val();
        o.backgroundColor = jQuery(".BR-jpgBgInput").val();
        o.drawBackground = o.type == "jpg";
        o.mime = o.type == "png" ? "image/png" : "image/jpeg";
        return o;
    ...
    this.getMainHtml = function () {
        ...
        // this.mainHtml += '<div class="BR-commandBtnArrow BR-commandSaveSub"  title=""></div>';
        ...
        this.mainHtml += '  <div class="BR-commandBtn BR-commandOutputSettings"  title="' + this.getLang("output_settings_tooltip") + '"></div>';
        this.mainHtml += '<div class="BR-saveOptionsHolder" style="display:none;">';
        this.mainHtml += this.getLang("output_image_configuration") + '<br/>';

        this.mainHtml += '<label><input type="radio" name="saveas" class="BR-saveAsPng" ' + (this.config.saveAs == "png" ? "checked" : "") + '></input>' + this.getLang("output_png_type") + '</label><br/>';
        this.mainHtml += '<label><input type="radio" name="saveas" class="BR-saveAsJpg" ' + (this.config.saveAs == "jpg" ? "checked" : "") + '></input>' + this.getLang("output_jpg_type") + '</label>';
        this.mainHtml += ', ' + this.getLang("output_jpg_quality") + ' <input type="number" min="1" max="100" class="BR-jpgQualityInput" value="' + this.config.saveAsJpgQuality + '"></input>';
        this.mainHtml += ', ' + this.getLang("output_background") + ' <input type="text" class="BR-jpgBgInput" value="' + this.config.saveAsBackground + '"></input><div class="BR-colorPickerHolderSaveAs"><div style="position: absolute;" class="BR-colorPickerSaveAs" ></div></div>';
        this.mainHtml += '</div>';
    ...
    this.drawCrop = function () {
        ...
        maskCanvas.width = this.rightCanvasDraw.width;
        maskCanvas.height = this.rightCanvasDraw.height;
        ...
        if (jQuery(".BR-autoCropCB").is(":checked")) {
            var b = this.bre.getAutoCropBounds(this.transparentImageCanvas.getContext("2d").getImageData(0, 0, this.leftImage.width, this.leftImage.height));
            this.setCropSize(b.x, b.y, b.width, b.height);
        } else {
            var b = this.getCropSize();
        }


        var _r = this.renderImageBounds;

        maskCtx.fillRect(_r.x + b.x * this.totalZoom, _r.y + b.y * this.totalZoom, b.width * this.totalZoom, b.height * this.totalZoom);

        if (b.width != 0 && this.converted) {
    ...
    this.getOutputImageCanvas = function () {
        ...
        outputCanvas.width = cropR.width;
        outputCanvas.height = cropR.height;

        var outputConfig = this.getOutputConfig();

        if (outputConfig.drawBackground) {
        ...
        if (this.transparentImageCanvasEdgeEffect)
            outputCanvas.getContext("2d").drawImage(this.transparentImageCanvasEdgeEffect, -cropR.x, -cropR.y, this.leftImage.width, this.leftImage.height);
        if (this.transparentImageCanvas)
            outputCanvas.getContext("2d").drawImage(this.transparentImageCanvas, -cropR.x, -cropR.y, this.leftImage.width, this.leftImage.height);
    ...
    this.initializeLeftEvents = function () {
        ...
        this.eraserSubHandlerListener = function (event) {
            ...
            if (jQuery(".BR-eraserOptionsHolder").is(":visible")) {
                self.hideSubOptions();
                return;
            };
        ...
        this.magicEdgeSubHandlerListener = function (event) {
            ...
            if (jQuery(".BR-magicEdgeOptionsHolder").is(":visible")) {
                self.hideSubOptions();
                return;
            };
    ...
    this.hideSubOptions = function () {
        ...
        jQuery(".BR-saveOptionsHolder").hide();
    }
    ...
    this.updateUndoRedoBtn = function () {
        ...
        if (this.drawArrAllUndo.length > 0) {
            jQuery(".BR-commandRedo").removeClass("BR-disabledBtn");
        } else {
            jQuery(".BR-commandRedo").addClass("BR-disabledBtn");
        }

        if (this.drawArrAll.length > 0) {
            jQuery(".BR-commandUndo").removeClass("BR-disabledBtn");
        } else {
            jQuery(".BR-commandUndo").addClass("BR-disabledBtn");
        }
    ...
    this.undo = function () {
        if (this.drawArrAll.length == 0)
            return;
        this.drawArrAllUndo.push(this.drawArrAll.pop());
        this.fitImages();
    ...
    this.redo = function () {
        if (this.drawArrAllUndo.length == 0)
            return;
        this.drawArrAll.push(this.drawArrAllUndo.pop());
        this.fitImages();
    ...
    this.autoCropCBChanged = function () {
        jQuery(".BR-autoCropCB").removeAttr("checked");
        this.fitImages(true);
    }
    ...
    this.setFeatherFilter = function (featherFilter) {
        ...
        this.config.featherFilter = parseInt(featherFilter);
    ...
    this.setBlurFilter = function (blurFilter) {
        ...
        this.config.blurFilter = blurFilter;
    ...
    this.resizeInterface = function (viewType) {
        ...
        if (this.viewType == "DUPLEX") {
            ...
            this.imageHolder.style.height = this.dropInfo.style.height = this.leftImageHolder.style.height = this.rightImageHolder.style.height = _h + "px";

            var _w = Math.floor(((this.imageHolder.offsetWidth - gap) / 2));

            this.dropInfo.style.width = this.leftImageHolder.style.width = this.rightImageHolder.style.width = _w + "px";
            ...
            this.dropInfo.style.width = this.leftImageHolder.style.width = _w + "px";
            ...
            this.dropInfo.style.width = this.rightImageHolder.style.width = _w + "px";
